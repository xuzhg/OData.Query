...
public class ApplyParser : QueryBinder, IApplyParser
{
    ...
    public virtual async ValueTask<ApplyClause> ParseAsync(ReadOnlyMemory<char> apply, QueryParserContext context)
    {
        if (apply.IsEmpty)
        {
        ...
        IApplyTokenizer applyTokenizer = context.GetOrCreateApplyTokenizer();
        ApplyToken applyToken = await applyTokenizer.TokenizeAsync(apply, context.TokenizerContext);
        if (applyToken == null)
        {
